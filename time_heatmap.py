...
def set_data(df):
    ...
    df = df.drop(['kingdom', 'phylum', 'class', 'order', 'family',
                  'oxygen', 'replicate', 'length'], axis=1)
...
def set_plot(df):
    ...
    for y in weeks:
        for m in bact:
            ...
            if abundance >= 0.3:
                ...
                colorx = "#75968f"
            color.append(colorx)
    source = ColumnDataSource(data=dict(name=name, week=week,
                                        color=color, rate=rate))
    ...
    p.select_one(HoverTool).tooltips = [('date', '@name @week'),
                                        ('rate', '@rate')]
